...
const ACTIVE_TRAINING_JOB_STATUSES = new Set([
  'queued',
  'pending',
  'running',
  'provisioning',
...
function parseRunNameFromCheckpointPrefix(prefix: string): string | null {
...
function resolveCheckoutStatus(input: {
  job: TrainingJob
  ...
  if (manualPromoted) return 'manual_promoted'
  if (validationInProgress) return 'validating'
  if (validationPassed) {
    if (adoptionMode === 'candidate') return 'candidate_ready'
    if (adoptionMode === 'keep_current') return 'kept_current'
    return 'promoted'
  }
...
function buildFallbackCheckoutSearch(job: TrainingJob): TrainingCheckoutSearch {
...
export function getTrainingCheckoutSearch(job: TrainingJob): TrainingCheckoutSearch {
  if (!job.checkout_search) {
    job.checkout_search = buildFallbackCheckoutSearch(job)
  }
  return job.checkout_search
}

export function isActiveTrainingJobStatus(status: string): boolean {
  return ACTIVE_TRAINING_JOB_STATUSES.has(status)
}

export function needsTrainingValidationFollowup(job: TrainingJob): boolean {
  return getTrainingCheckoutSearch(job).status === 'validating'
}

export function shouldWatchTrainingJob(job: TrainingJob): boolean {
  return isActiveTrainingJobStatus(job.status) || needsTrainingValidationFollowup(job)
}

export function getTrainingJobDisplayStatus(job: TrainingJob): string {
  if (isActiveTrainingJobStatus(job.status)) {
    return job.status
  }

  const checkoutStatus = getTrainingCheckoutSearch(job).status
  return checkoutStatus === 'pending' ? job.status : checkoutStatus
}
